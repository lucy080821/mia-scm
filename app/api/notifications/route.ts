  ...
  try {
    ...
    const [newOrders, pendingPOs, delayedDeliveries, doneOrders, doneDeliveries, allInv, lowProducts] =
      await Promise.all([
        ...
        // Giao hàng đang trên đường > 24h (có thể trễ)
        supabaseAdmin
          .from('deliveries')
          .select('code, driver_name, created_at')
          .eq('tenant_id', tenantId)
        ...
        supabaseAdmin
          .from('deliveries')
          .select('code, driver_name, updated_at')
          .eq('tenant_id', tenantId)
    ...
    // Đơn mua hàng chờ duyệt
    for (const po of pendingPOs.data ?? []) {
    ...
    for (const d of delayedDeliveries.data ?? []) {
      notifs.push({
        id: `del-late-${d.code}`,
        type: 'delivery',
    ...
    // Đơn hàng hoàn thành
    for (const o of doneOrders.data ?? []) {
      const cust = (o.customer as { name?: string } | null)?.name ?? ''
      notifs.push({
        id: `ord-done-${o.code}`,
        type: 'order',
    ...
    for (const d of doneDeliveries.data ?? []) {
      notifs.push({
        id: `del-done-${d.code}`,
        type: 'delivery',
