...
interface StockIssue {
  id: string; code: string; sales_order_code: string
  ...
  return {
    ...
    items: rawItems.map(it => {
      ...
      return {
        ...
        lots: [],
        selectedLot: (it.lot_number as string) ?? '',
        pickQty: 0,
      }
    }),
  }
...
function PickingModal({ issue, onClose, onComplete, onStart }: {
  ...
  // Fetch live inventory lots for FEFO picking
  useEffect(() => {
    if (!issue.warehouse_id) { setLoading(false); return }
    const productIds = issue.items.map(it => it.product_id)
    supabase
      .from('inventory')
      ...
      .then(({ data }) => {
        ...
        setItems(prev => prev.map(it => ({
          ...it,
          lots: inv
            .filter(row => row.product_id === it.product_id)
            .map(row => ({
              lot_number: row.lot_number || '',
              quantity: row.quantity,
              expiry_date: row.expiry_date || '',
            })),
          selectedLot: inv.find(row => row.product_id === it.product_id)?.lot_number || it.selectedLot,
        })))
        setLoading(false)
      })
  }, [issue.warehouse_id, issue.items])
  ...
  const allDone = items.every(it => it.picked >= it.required)
...
function ConfirmedOrdersPanel() {
  ...
  const transition = async (id: string, to: 'picking' | 'picked') => {
    setUpdating(id)
      ...
      <div className="space-y-2">
        {orders.map(o => (
          <div key={o.id} className="bg-white rounded-xl border border-amber-100 px-4 py-3 flex items-center justify-between gap-4">
            ...
            <div className="shrink-0">
              {o.status === 'confirmed' && (
                <button onClick={() => transition(o.id, 'picking')} disabled={updating === o.id}
              ...
              {o.status === 'picking' && (
                <button onClick={() => transition(o.id, 'picked')} disabled={updating === o.id}
                  ...
                  {updating === o.id ? '...' : 'Xác nhận xuất kho'}
                </button>
              )}
            </div>
          </div>
                        ...
                        {r.sales_order_code && r.sales_order_code !== '—' && (
                          synced.has(r.id) ? (
                            ...
                            <button
                              ...
                              title="Tạo đơn vận chuyển nếu chưa có"
